...
exports.saveShipmentDistribution = exports.getShipmentDistribution = exports.getMmlForDistribution = exports.deleteSvod = exports.updateSvodLine = exports.refreshSvod = exports.saveSvod = exports.getSvodByDate = void 0;
 ...
 */
const getSvodByDate = async (req, res) => {
    try {
...
exports.getSvodByDate = getSvodByDate;
/**
 ...
 */
async function getClosestPreviousEOD(svodDate) {
 ...
 */
async function buildSvodPreview(svodDate) {
    ...
    const totalQtyBySupplier = new Map();
...
exports.saveSvod = saveSvod;
/**
 * Обновить существующий свод
 */
async function updateExistingSvod(res, svodId, username, lines, supplierCols, supplierValues) {
    try {
        await prisma.$transaction(async (tx) => {
            ...
            const productIdToNewLineId = new Map();
 ...
 */
const refreshSvod = async (req, res) => {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            if (freshData.supplierCols.length > 0) {
                await tx.svodSupplierCol.createMany({
                    data: freshData.supplierCols.map((col) => ({
                        svodId,
                        colIndex: col.colIndex,
                        supplierId: col.supplierId,
                        supplierName: col.supplierName,
            ...
            if (freshData.supplierValues.length > 0) {
                await tx.svodSupplierValue.createMany({
                    data: freshData.supplierValues.map((val) => ({
                        svodId,
                        productId: val.productId,
                        supplierId: val.supplierId,
                        purchaseQty: val.purchaseQty || 0
...
exports.refreshSvod = refreshSvod;
/**
 * Обновить строку свода (ручные правки)
 * PATCH /api/svod/lines/:lineId
 */
const updateSvodLine = async (req, res) => {
    try {
...
exports.updateSvodLine = updateSvodLine;
/**
 * Удалить СВОД
 * DELETE /api/svod/:id
 */
const deleteSvod = async (req, res) => {
    try {
        const svodId = Number(req.params.id);
 ...
 */
const getMmlForDistribution = async (req, res) => {
    try {
...
exports.getMmlForDistribution = getMmlForDistribution;
/**
 * Получить распределение веса для строки свода
 * GET /api/svod/lines/:lineId/distribution
 */
const getShipmentDistribution = async (req, res) => {
    try {
        const lineId = Number(req.params.lineId);
...
exports.getShipmentDistribution = getShipmentDistribution;
/**
 ...
 * }
 */
const saveShipmentDistribution = async (req, res) => {
    try {
        const lineId = Number(req.params.lineId);
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const validDistributions = distributions.filter((d) => d.qty > 0 && d.productId);
            ...
            const newProductIds = new Set(validDistributions.map((d) => d.productId));
...
exports.saveShipmentDistribution = saveShipmentDistribution;
