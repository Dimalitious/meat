...
const importData = async (req, res) => {
    try {
        ...
        // --- 1. ENSURE DEFAULTS ---
        const defaultDistrict = await db_1.prisma.district.upsert({
            where: { code: 'GENERAL' },
            update: {},
            create: { code: 'GENERAL', name: 'Общий район' }
        });
        const defaultManager = await db_1.prisma.manager.upsert({
            where: { code: 'GENERAL' },
            update: {},
        ...
        if (workbook.Sheets[productSheetName]) {
            const rows = XLSX.utils.sheet_to_json(workbook.Sheets[productSheetName]);
        ...
        if (workbook.Sheets['sup']) {
            const rows = XLSX.utils.sheet_to_json(workbook.Sheets['sup']);
            rows.forEach(row => {
                const name = row['торговые точки'];
                if (name)
                    customerNames.add(String(name).trim());
            });
        }
        // From 'VIP клиенты'
        if (workbook.Sheets['VIP клиенты']) {
            const rows = XLSX.utils.sheet_to_json(workbook.Sheets['VIP клиенты']);
            rows.forEach(row => {
        ...
        for (const name of customerNames) {
            ...
            try {
                await db_1.prisma.customer.upsert({
                    where: { code },
                    update: {
                        name,
                ...
                try {
                    await db_1.prisma.customer.create({
                        data: {
                            code: newCode,
                            name,
                            districtId: defaultDistrict.code,
