...
exports.getOrderEvents = exports.returnFromAssemblyBatch = exports.returnFromAssembly = exports.startAssembly = exports.sendToRework = exports.syncToOrders = exports.bulkDeleteSummaryOrders = exports.deleteSummaryOrder = exports.updateSummaryOrder = exports.bulkCreateSummaryOrders = exports.createSummaryOrder = exports.getFilterOptions = exports.getSummaryOrders = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
// Get summary orders with filters and pagination - OPTIMIZED
const getSummaryOrders = async (req, res) => {
    try {
        const { date, customerId, productId, category, district, managerId, status, page = '1', limit = '100' // Увеличили лимит для меньшего количества запросов
         } = req.query;
...
exports.getSummaryOrders = getSummaryOrders;
...
const getFilterOptions = async (req, res) => {
    try {
        ...
        const categories = catData.map(c => c.category).filter(Boolean);
        const districts = distData.map(d => d.district).filter(Boolean);
...
const createSummaryOrder = async (req, res) => {
    try {
        const { shipDate, paymentType, customerId, customerName, productId, productCode, productFullName, category, shortNameMorning, priceType, price, shippedQty, orderQty, distributionCoef, weightToDistribute, managerId, managerName, district, pointAddress, status } = req.body;
...
exports.createSummaryOrder = createSummaryOrder;
// BULK CREATE - for fast Excel import
const bulkCreateSummaryOrders = async (req, res) => {
    try {
...
exports.bulkCreateSummaryOrders = bulkCreateSummaryOrders;
// Update summary order entry
const updateSummaryOrder = async (req, res) => {
    try {
        const { id } = req.params;
        const data = { ...req.body };
        const user = req.user;
        ...
        if (data.shippedQty !== undefined)
            data.shippedQty = Number(data.shippedQty);
        if (data.orderQty !== undefined)
            data.orderQty = Number(data.orderQty);
        if (data.distributionCoef !== undefined)
            data.distributionCoef = Number(data.distributionCoef);
        if (data.weightToDistribute !== undefined)
            data.weightToDistribute = Number(data.weightToDistribute);
...
exports.updateSummaryOrder = updateSummaryOrder;
// Delete summary order entry
const deleteSummaryOrder = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.deleteSummaryOrder = deleteSummaryOrder;
// BULK DELETE - mass deletion by IDs or by date
const bulkDeleteSummaryOrders = async (req, res) => {
    try {
        const { ids, date, excludeSynced } = req.body;
        // If specific IDs provided - delete those
        if (ids && Array.isArray(ids) && ids.length > 0) {
            const where = { id: { in: ids.map((id) => Number(id)) } };
...
exports.bulkDeleteSummaryOrders = bulkDeleteSummaryOrders;
...
exports.syncToOrders = syncToOrders;
// Send to rework
const sendToRework = async (req, res) => {
    try {
        const { idn } = req.body;
        if (!idn) {
...
exports.sendToRework = sendToRework;
// ============================================
...
exports.startAssembly = startAssembly;
// Вернуть со сборки - POST /api/summary-orders/:id/assembly/return
const returnFromAssembly = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.returnFromAssembly = returnFromAssembly;
// Массовый возврат со сборки - POST /api/summary-orders/assembly/return-batch
const returnFromAssemblyBatch = async (req, res) => {
    try {
...
exports.returnFromAssemblyBatch = returnFromAssemblyBatch;
