...
exports.FIFO_TOLERANCE_KG = void 0;
exports.buildFifoAllocations = buildFifoAllocations;
exports.voidAllocations = voidAllocations;
exports.createAllocationRecords = createAllocationRecords;
exports.initializeQtyRemaining = initializeQtyRemaining;
exports.recalculateQtyRemaining = recalculateQtyRemaining;
exports.getAffectedPurchaseItemIds = getAffectedPurchaseItemIds;
const client_1 = require("@prisma/client");
// Constants
const ABS_TOL_KG = new client_1.Prisma.Decimal('0.3'); // 300g tolerance
/**
 ...
 */
async function buildFifoAllocations(tx, productId, dayEnd, documentWeight, excludeSourceId, excludeSourceType) {
    ...
    for (const pi of purchaseItems) {
        if (remainingNeed.lte(0))
            break;
        const available = new client_1.Prisma.Decimal(pi.qtyRemaining);
        if (available.lte(0))
            continue;
        const take = client_1.Prisma.Decimal.min(available, remainingNeed);
    ...
    const totalAllocated = allocations.reduce((sum, a) => sum.add(a.qtyAllocated), new client_1.Prisma.Decimal(0));
 ...
 */
async function voidAllocations(tx, sourceType, sourceId, voidReason) {
 ...
 */
async function createAllocationRecords(tx, sourceType, sourceId, productId, allocations) {
    if (allocations.length === 0)
        return;
    const now = new Date();
 ...
 */
async function initializeQtyRemaining(tx, purchaseItemId) {
    await tx.$executeRaw `
    UPDATE "PurchaseItem" 
 ...
 * @param purchaseItemId - Purchase item ID
 */
async function recalculateQtyRemaining(tx, purchaseItemId) {
    const item = await tx.purchaseItem.findUnique({
        where: { id: purchaseItemId },
        select: { qty: true }
    });
    if (!item)
        throw new Error(`PurchaseItem ${purchaseItemId} not found`);
    ...
    const totalAllocated = allocatedAgg._sum.qtyAllocated ?? new client_1.Prisma.Decimal(0);
    const qtyRemaining = new client_1.Prisma.Decimal(item.qty).sub(totalAllocated);
 ...
 * @param sourceId - Document ID
 */
async function getAffectedPurchaseItemIds(tx, sourceType, sourceId) {
    const allocations = await tx.productionAllocation.findMany({
...
exports.FIFO_TOLERANCE_KG = ABS_TOL_KG;
