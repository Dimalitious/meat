...
async function importData() {
    try {
        const fullPath = path_1.default.resolve(process.cwd(), FILE_PATH);
        ...
        if (data.districts) {
            ...
            for (const d of data.districts) {
                if (!d.district_id)
                    continue;
                await db_1.prisma.district.upsert({
        ...
        if (data.managers) {
            ...
            for (const m of data.managers) {
                if (!m.manager_id)
                    continue;
                await db_1.prisma.manager.upsert({
        ...
        if (data.suppliers) {
            ...
            for (const s of data.suppliers) {
                if (!s.supplier_id)
                    continue;
                await db_1.prisma.supplier.upsert({
        ...
        if (data.products) {
            ...
            for (const p of data.products) {
                if (!p.product_code)
                    continue;
                await db_1.prisma.product.upsert({
                    where: { code: String(p.product_code) },
                    update: {
                        name: p.product_name,
                        altName: p.alt_name,
                        priceListName: p.short_name_morning,
                        category: p.category,
