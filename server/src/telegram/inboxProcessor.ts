...
const PROCESS_INTERVAL_MS = 5_000; // 5 сек
...
function looksLikeOrder(text: string): boolean {
    ...
    const hasNumberLine = lines.some(line =>
        /\d+/.test(line) && /[а-яА-ЯёЁa-zA-Z]{2,}/.test(line)
    );
    // Или ключевые слова
 ...
 */
function parseOrderLines(text: string): Array<{
    rawText: string;
    title: string | null;
    ...
    for (const line of lines) {
        ...
        const match = line.match(
            /^(.+?)\s+(\d+(?:[.,]\d+)?)\s*(кг|kg|шт|шток|штук|pcs|кор|box|коробк\w*)?\.?\s*$/i
        );
        ...
        if (match) {
            ...
            if (['кг', 'kg'].includes(unitRaw)) unit = 'KG';
            else if (['шт', 'шток', 'штук', 'pcs'].includes(unitRaw)) unit = 'PCS';
            else if (['кор', 'box'].some(u => unitRaw.startsWith(u))) unit = 'BOX';
            else unit = 'KG'; // default
...
async function matchProducts(
    items: Array<{ rawText: string; title: string | null; quantity: number | null; unit: string | null }>,
    customerId: number,
): Promise<Array<{
    rawText: string;
    title: string | null;
    ...
    return items.map(item => {
        ...
        const cpMatch = customerProducts.find(cp =>
            cp.product.name.toLowerCase() === searchTerm ||
        ...
        const cpContains = customerProducts.find(cp =>
            cp.product.name.toLowerCase().includes(searchTerm) ||
            searchTerm.includes(cp.product.name.toLowerCase()) ||
            (cp.product.altName && (
                cp.product.altName.toLowerCase().includes(searchTerm) ||
        ...
        const containsMatch = allProducts.find(p =>
            p.name.toLowerCase().includes(searchTerm) ||
            searchTerm.includes(p.name.toLowerCase()) ||
            (p.altName && (
                p.altName.toLowerCase().includes(searchTerm) ||
 ...
 */
export async function createDraftFromInbox(
    inboxId: number,
    chatId: bigint,
): Promise<{ draftId?: number; summary?: string; error?: string }> {
    const inbox = await prisma.telegramInbox.findUnique({ where: { id: inboxId } });
    if (!inbox) return { error: 'Сообщение не найдено' };
    if (inbox.status !== 'PROMPTED') return { error: 'Уже обработано' };
...
async function processNewMessages(): Promise<void> {
    ...
    try {
        ...
        await prisma.telegramInbox.updateMany({
            where: {
                status: 'PROMPTED',
        ...
        const newMessages = await prisma.telegramInbox.findMany({
            where: { status: 'NEW' },
        ...
        for (const msg of newMessages) {
            try {
                ...
                await bot.api.sendMessage(
                    Number(msg.chatId),
                    `📋 Похоже на заказ. Создать черновик?`,
                    {
...
export function startInboxProcessor(): void {
    if (processorRunning || !bot) return;
...
export function stopInboxProcessor(): void {
