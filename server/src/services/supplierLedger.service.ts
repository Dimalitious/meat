...
// ============================================
// Sync helpers: каждая функция вызывается
// ВНУТРИ транзакции после создания/обновления/удаления документа
// ============================================
 ...
 */
export async function syncPurchaseLedger(tx: TX, purchaseId: number): Promise<void> {
    ...
    for (const s of sums) {
        const credit = s._sum.amount ?? new Prisma.Decimal(0);
 ...
 */
export async function syncSupplierReturnLedger(tx: TX, returnId: number): Promise<void> {
 ...
 */
export async function syncSupplierPaymentLedger(tx: TX, paymentId: number): Promise<void> {
...
export async function upsertOpeningBalance(
    ...
    await tx.supplierLedger.upsert({
        ...
        create: {
            ...
            opDate: date,
            debit: isDebt ? new Prisma.Decimal(0) : new Prisma.Decimal(Math.abs(amount)),
            credit: isDebt ? new Prisma.Decimal(amount) : new Prisma.Decimal(0),
            isHidden: false,
        },
        update: {
            opDate: date,
