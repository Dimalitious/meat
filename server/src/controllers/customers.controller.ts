...
export const importCustomers = async (req: Request, res: Response) => {
    try {
        ...
        const customersData: any[] = XLSX.utils.sheet_to_json(customersSheet, { header: 1 });
        ...
        const cardsData: any[] = cardsSheet ? XLSX.utils.sheet_to_json(cardsSheet, { header: 1 }) : [];
        ...
        for (let i = 1; i < customersData.length; i++) {
            ...
            try {
                ...
                if (existing) {
                    await prisma.customer.update({ where: { code }, data: customerData });
        ...
        const cardCache = new Map<string, number>(); // customerCode_cardName -> cardId
        ...
        for (let i = 1; i < cardsData.length; i++) {
            ...
            try {
                ...
                if (!cardId) {
                    ...
                    let card = await prisma.customerCard.findFirst({
                ...
                await prisma.customerCardItem.upsert({
                    where: {
...
export const exportCustomersWithPhotos = async (req: Request, res: Response) => {
    try {
        ...
        for (const photo of photoUrls) {
            try {
                const response = await axios.get(photo.url, { responseType: 'arraybuffer', timeout: 10000 });
...
export const importCustomersFromZip = async (req: Request, res: Response) => {
    try {
        if (!req.file) {
            return res.status(400).json({ error: 'No file uploaded' });
        }
                    ...
                    if (fs.existsSync(photosDir)) {
                        for (let photoIdx = 0; photoIdx < photoFiles.length; photoIdx++) {
                            ...
                            if (fs.existsSync(photoPath)) {
                                ...
                                const existingPhoto = await prisma.customerCardItemPhoto.findFirst({
                                    where: { itemId: item.id, sortOrder: photoIdx }
                                });

                                if (existingPhoto) {
                                    await prisma.customerCardItemPhoto.update({
                                        where: { id: existingPhoto.id },
                                        data: { url: dataUrl }
                                    });
                                } else {
                                    await prisma.customerCardItemPhoto.create({
                                        data: {
                                            itemId: item.id,
                                            url: dataUrl,
                                            sortOrder: photoIdx
